...
\begin{frame}{Introduction}
  \begin{itemize}
    ...
    \item Importance of Immune System
    \item Selective Pressure and Challenges
    \item Why Mathematical Modeling is Important
  \end{itemize}
\end{frame}

\begin{frame}{Antibiotic Resistance}

	\begin{itemize}
		\item Bacteria becoming resistant
		
	\textbf{Why this happens?}
		\item Antibiotic Abuse
		\item Early Stop of Medication
		\item Prescription Error (Dosage and Medication)
	\end{itemize}
\end{frame}

\begin{frame}{Importance of Immune System}
	\begin{itemize}
		\item Immune System can put up a sturdy defense
		\item Severe Infection weakens Immune System
		\item Mild ARB Infection Becomes Fatal When Immune System is Weak
		\item \textbf{WHO} Warns \textcolor{red}{25 million} Death by 2050
	\end{itemize}
\end{frame}	


\begin{frame}{Selective Pressure and Challenges}
	\begin{itemize}
		\item Difficulties of Curing ARB Infection
		\item Incorrect Prescription leads to selective pressure
		\item Small Change of Dosage can make a Considerable Difference
		\item Epidemiological Models shows less abuse affects growth of resistance in population level
	\end{itemize}

\end{frame}

\begin{frame}{Why Mathematical Modeling is Important}
	\begin{itemize}
		\item we ask: 
		\begin{itemize}
			\item In what circumstances Antibiotic solely kills infection? 
			\item When combined interventions i.e. Immunity enhancement or multi medication--is needed?
		\end{itemize}
		\item Mathematical models show
		\begin{itemize}
			\item simulation of various therapeutic scenarios
			\item Overdosage and Underdosage effects
		\end{itemize}
	\end{itemize}
...
\begin{frame}{Parameter and Term Definitions}
  \begin{description}
    ...
    \item[$\mu$]: absorption rate of Antibiotic 
    \item[$\eta_S$ and $\eta_R$]: reproduction rate of $S$ and $R$
    \item[$K$]: carrying capacity (Limiting the reproduction) 
    \item[$\beta$]: transfer rate of resistant gene
    \item[$\Gamma$]: Immune System's activity
    \item[$P_{\text{max}}$]: limit of proliferation of immune cells
    \item[$\Psi$ and $\Phi$]: functions in \(\mathcal{C}^1(\mathbb{R}_+)\) to adjust $P$ with biological 
  \end{description}
\end{frame}
...
\[ 
  \begin{array}{ccccc}
    a = \frac{A}{\Lambda/\mu}=\frac{\mu A}{\Lambda} & s = \frac{S}{K} & r = \frac{R}{K} & p = \frac{P}{P_{\max}} & \\
...
\section{Equilibria}
\begin{frame}
  Analyzing reveals 4 Equilibria - stable
  \begin{itemize}
...
\begin{frame}[shrink=20]{deriving Equilibria}
  \begin{columns}
    \column{0.65\textwidth}
      \begin{itemize}
          \item Equilibria $\equiv$ Zero Change $\equiv$  \( \dot{\mathbf{X}} = \vec{0} \)
...
\begin{frame}[fragile,allowframebreaks]{Deriving Jacobian}
...
\begin{frame}[fragile,allowframebreaks]{ODE Numeric Solve}
